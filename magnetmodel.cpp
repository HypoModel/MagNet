...
MagNetModel::MagNetModel(MagNetMod *magnetmod)
	: ModThread(magnetmod->modbox, magnetmod->mainwin), neurons(magnetmod->modneurons)
{
	mod = magnetmod;
...
int MagNetModel::InputGen()
{
	...
	for(inpcell=0; inpcell<inputcells; inpcell++) {
		epspt = -log(1 - rng.uniform_open01()) / erate;   // initial epspt value, could just be 0
		ipspt = -log(1 - rng.uniform_open01()) / irate;
		...
		for(t=0; t<numsteps; t++) {
			...
			while(epspt < hstep) {
				nepsp++;
				epspt = -log(1 - rng.uniform_open01()) / erate + epspt;
			}
			epspt = epspt - hstep;
			...
			while(ipspt < hstep) {
				nipsp++;
				ipspt = -log(1 - rng.uniform_open01()) / irate + ipspt;
			}
			ipspt = ipspt - hstep;
			if(nipsp > 0) {
				// add input
...
void MagNetModel::Export2file(int steps, wxString filename, datdouble vector2print)
{
	float tempvalue;
...
void MagNetModel::SecretionAnalysis()
{
	int i, datacount;
